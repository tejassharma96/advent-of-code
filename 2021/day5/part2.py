...
class Vent:
    ...
    def range(self):
        if self.horizontal():
            ...
            x_step = 1 if self.p1.x < self.p2.x else -1
            y_step = 1 if self.p1.y < self.p2.y else -1
            x_range = range(self.p1.x, self.p2.x + x_step, x_step)
            y_range = range(self.p1.y, self.p2.y + y_step, y_step)
